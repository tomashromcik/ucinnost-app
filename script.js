// Generátor slovních úloh na účinnost (výkony W/kW/MW) — verze s null-safety

const $ = (id)=> document.getElementById(id);
const screen = document.getElementById('screen'); // může být zatím prázdný
const F = { W:1, kW:1000, MW:1_000_000 };
const fmtW = (w)=> w>=1_000_000? (w/1_000_000).toFixed(3)+' MW' : w>=1000? (w/1000).toFixed(3)+' kW' : w.toFixed(2)+' W';

let step = 0;
let problem = null;
let stats = { ok:0, err:0, accSum:0, accN:0 };

const DEVICES=[
  {id:'zarovka',name:'Žárovka'}, {id:'motor',name:'Motor'},
  {id:'turbina',name:'Turbína'}, {id:'cerpadlo',name:'Čerpadlo'},
  {id:'pec',name:'Elektrická pec'}
];

function makeProblem(){
  const dev = DEVICES[Math.floor(Math.random()*DEVICES.length)];
  const type = ['eta','puse','pin'][Math.floor(Math.random()*3)];
  const PinW = [200, 5000, 60, 2_000_000][Math.floor(Math.random()*4)];
  const eta = [35,55,60,75,85][Math.floor(Math.random()*5)];
  const PuseW = PinW * (eta/100);
  const pick = (w)=> w>=1_000_000? {v:w/1_000_000,u:'MW'} : w>=1000? {v:w/1000,u:'kW'} : {v:w,u:'W'};
  return {
    device: dev, type, PinW, PuseW, eta,
    Pin: pick(PinW), Puse: pick(PuseW),
    text:
      type==='eta'
        ? `${dev.name} odebírá celkový příkon ${pick(PinW).v.toFixed(3)} ${pick(PinW).u}. Užitečný výkon je ${pick(PuseW).v.toFixed(3)} ${pick(PuseW).u}. Urči účinnost zařízení.`
      : type==='puse'
        ? `${dev.name} má účinnost ${eta}% a odebírá celkový příkon ${pick(PinW).v.toFixed(3)} ${pick(PinW).u}. Urči užitečný výkon.`
        : `${dev.name} má účinnost ${eta}% a dodává užitečný výkon ${pick(PuseW).v.toFixed(3)} ${pick(PuseW).u}. Urči celkový příkon.`,
    ask:
      type==='eta'  ? 'Vypočítej účinnost η v procentech.' :
      type==='puse' ? 'Vypočítej Pᵤ (užitečný výkon).' :
                      'Vypočítej Pᵢₙ (celkový příkon).'
  };
}

// ——— vykreslení kroků ———
function setStepVisual(){
  document.querySelectorAll('.step').forEach((el,i)=> el.classList.toggle('active', i===step));
  const back = $('#btnBack'); if (back) back.disabled = (step===0);
  const next = $('#btnNext'); if (next) next.disabled = (step===3);
}

function render(){
  setStepVisual();
  if (!screen) return;
  screen.innerHTML = '';

  if (step===0){
    screen.innerHTML = `
      <h2>1. Zadání</h2>
      <p>${problem.text}</p>
      <div class="badge wip">Úkol: ${problem.ask}</div>
      <div class="hr"></div>
      <p class="small muted">Tip: vyznač si dané a neznámé veličiny. Pracujeme s výkony (W, kW, MW).</p>`;
    return;
  }

  if (step===1){
    screen.innerHTML = `
      <h2>2. Zápis</h2>
      <div class="grid2">
        <div>
          <label>Celkový příkon P<sub>in</sub></label>
          <div class="row gap">
            <input id="pinVal" class="input" type="number" inputmode="decimal" placeholder="hodnota">
            <select id="pinUnit" class="input"><option>W</option><option>kW</option><option>MW</option></select>
          </div>
        </div>
        <div>
          <label>Užitečný výkon P<sub>užit</sub></label>
          <div class="row gap">
            <input id="puseVal" class="input" type="number" inputmode="decimal" placeholder="hodnota">
            <select id="puseUnit" class="input"><option>W</option><option>kW</option><option>MW</option></select>
          </div>
        </div>
      </div>
      <p class="small muted">Opíšete hodnoty ze zadání. Pokud v zadání chybí, pole nechte prázdné.</p>
      <div id="writeMsg" class="small"></div>`;

    const pinU = $('#pinUnit'), pinV = $('#pinVal');
    const puseU = $('#puseUnit'), puseV = $('#puseVal');
    if (pinU) pinU.value = problem.Pin.u;
    if (puseU) puseU.value = problem.Puse.u;
    if (pinV)  pinV.placeholder  = (problem.type==='pin'  ? '?' : problem.Pin.v.toFixed(3));
    if (puseV) puseV.placeholder = (problem.type==='puse' ? '?' : problem.Puse.v.toFixed(3));
    return;
  }

  if (step===2){
    let inner = '';
    if (problem.type==='eta'){
      inner = `
        <label>Výsledek – účinnost η (%)</label>
        <input id="eta" class="input" type="number" inputmode="decimal" placeholder="např. 75">
        <p class="small muted">η = P<sub>užit</sub> / P<sub>in</sub> × 100 %</p>`;
    } else if (problem.type==='puse'){
      inner = `
        <label>Výsledek – P<sub>užit</sub></label>
        <div class="row gap">
          <input id="puseCalc" class="input" type="number" inputmode="decimal" placeholder="hodnota">
          <select id="puseCalcUnit" class="input"><option>W</option><option>kW</option><option>MW</option></select>
        </div>
        <p class="small muted">P<sub>užit</sub> = η · P<sub>in</sub></p>`;
    } else {
      inner = `
        <label>Výsledek – P<sub>in</sub></label>
        <div class="row gap">
          <input id="pinCalc" class="input" type="number" inputmode="decimal" placeholder="hodnota">
          <select id="pinCalcUnit" class="input"><option>W</option><option>kW</option><option>MW</option></select>
        </div>
        <p class="small muted">P<sub>in</sub> = P<sub>užit</sub> / η</p>`;
    }
    screen.innerHTML = `<h2>3. Výpočet</h2>${inner}<div id="calcMsg" class="small"></div>`;
    const u1 = $('#puseCalcUnit'); if (u1) u1.value = problem.Puse.u;
    const u2 = $('#pinCalcUnit');  if (u2) u2.value = problem.Pin.u;
    return;
  }

  if (step===3){
    screen.innerHTML = `
      <h2>4. Odpověď</h2>
      <textarea id="ans" rows="3" class="input" placeholder="Zapíšete stručnou odpověď celou větou..."></textarea>
      <div class="small muted">Např.: „Účinnost motoru je 75 %.“</div>
      <div id="ansMsg" class="small"></div>`;
  }
}

function check(){
  if (step===1){
    const pinV = $('#pinVal'), pinU = $('#pinUnit');
    const puseV = $('#puseVal'), puseU = $('#puseUnit');
    const msgBox = $('#writeMsg');
    if (!pinV || !pinU || !puseV || !puseU || !msgBox) return false;

    let ok=true, msg=[];
    if (problem.type!=='pin'){
      const want = problem.Pin.v * F[problem.Pin.u];
      const got  = Number(pinV.value) * F[pinU.value];
      if (!(isFinite(got) && Math.abs(got - want) < Math.max(1e-6, want*0.001))) { ok=false; msg.push('P_in neodpovídá zadání.'); }
    } else if (pinV.value.trim()!==''){ ok=false; msg.push('P_in v zadání chybí – v zápisu ho nevyplňuj.'); }

    if (problem.type!=='puse'){
      const want = problem.Puse.v * F[problem.Puse.u];
      const got  = Number(puseV.value) * F[puseU.value];
      if (!(isFinite(got) && Math.abs(got - want) < Math.max(1e-6, want*0.001))) { ok=false; msg.push('P_užit neodpovídá zadání.'); }
    } else if (puseV.value.trim()!==''){ ok=false; msg.push('P_užit v zadání chybí – v zápisu ho nevyplňuj.'); }

    msgBox.innerHTML = ok ? '<span class="success">✅ Zápis odpovídá zadání.</span>'
                          : '<span class="error">❌ '+msg.join(' ')+'</span>';
    return ok;
  }

  if (step===2){
    const tolRel = 0.005; // 0,5 %
    const msgBox = $('#calcMsg'); if (!msgBox) return false;
    let ok=false, acc=0, message='';

    if (problem.type==='eta'){
      const etaEl = $('#eta'); const eta = etaEl ? Number(etaEl.value) : NaN;
      if (isFinite(eta)){ acc = 100 - Math.min(100, Math.abs(eta - problem.eta)); ok = Math.abs(eta - problem.eta) <= problem.eta*tolRel; }
      message = ok ? `✅ Správně. η = ${problem.eta.toFixed(1)} %.` : `❌ Nesouhlasí. Očekává se ~${problem.eta.toFixed(1)} %.`;
    } else if (problem.type==='puse'){
      const vEl = $('#puseCalc'), uEl = $('#puseCalcUnit');
      const gotW = (vEl && uEl) ? Number(vEl.value) * F[uEl.value] : NaN;
      const wantW = problem.PuseW;
      if (isFinite(gotW)){ acc = 100 - Math.min(100, Math.abs(gotW - wantW)/wantW*100); ok = Math.abs(gotW - wantW) <= wantW*tolRel; }
      message = ok ? `✅ Správně. P_užit = ${fmtW(wantW)}.` : `❌ Nesouhlasí. Očekává se ~${fmtW(wantW)}.`;
    } else {
      const vEl = $('#pinCalc'), uEl = $('#pinCalcUnit');
      const gotW = (vEl && uEl) ? Number(vEl.value) * F[uEl.value] : NaN;
      const wantW = problem.PinW;
      if (isFinite(gotW)){ acc = 100 - Math.min(100, Math.abs(gotW - wantW)/wantW*100); ok = Math.abs(gotW - wantW) <= wantW*tolRel; }
      message = ok ? `✅ Správně. P_in = ${fmtW(wantW)}.` : `❌ Nesouhlasí. Očekává se ~${fmtW(wantW)}.`;
    }

    msgBox.innerHTML = ok ? `<span class="success">${message}</span>` : `<span class="error">${message}</span>`;
    if (ok){ stats.ok++; stats.accSum += acc; stats.accN++; } else { stats.err++; }
    updateStats();
    return ok;
  }

  if (step===3){
    const ans = $('#ans'); const box = $('#ansMsg'); if (!ans || !box) return false;
    const txt = (ans.value || '').trim();
    const ok = txt.length > 0;
    let sample='';
    if (problem.type==='eta')  sample = `Účinnost ${problem.device.name.toLowerCase()} je ${problem.eta.toFixed(1)} %.`;
    if (problem.type==='puse') sample = `Užitečný výkon je přibližně ${fmtW(problem.PuseW)}.`;
    if (problem.type==='pin')  sample = `Celkový příkon je přibližně ${fmtW(problem.PinW)}.`;
    box.innerHTML = ok ? `<span class="success">✅ Odpověď zapsána.</span> <span class="badge ok">Vzorem: ${sample}</span>`
                       : `<span class="error">❌ Doplň slovní odpověď v jedné větě.</span>`;
    return ok;
  }
  return true;
}

function updateStats(){
  const okEl = $('#okCount'); if (okEl) okEl.textContent = stats.ok;
  const errEl = $('#errCount'); if (errEl) errEl.textContent = stats.err;
  const avgEl = $('#avgAcc'); if (avgEl) avgEl.textContent = stats.accN ? (stats.accSum/stats.accN).toFixed(1)+' %' : '–';
}

// ——— Ovládání ———
function wireControls(){
  const yn = (id, fn)=>{ const el = $(id); if (el) el.addEventListener('click', fn); };
  yn('btnNew',  ()=>{ problem = makeProblem(); step = 0; render(); });
  yn('btnReset',()=>{ stats = {ok:0,err:0,accSum:0,accN:0}; updateStats(); problem = makeProblem(); step = 0; render(); });
  yn('btnBack', ()=>{ if (step>0){ step--; render(); }});
  yn('btnNext', ()=>{ if (step<3){ step++; render(); }});
  yn('btnCheck',()=>{ check(); });
}

// ——— Start ———
(function start(){
  const y = $('#year'); if (y) y.textContent = new Date().getFullYear();
  problem = makeProblem();
  render();
  updateStats();
  wireControls();
})();
